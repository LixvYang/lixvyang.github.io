import{_ as p,r as e,o,c,b as s,d as n,a as i,e as a}from"./app-927a6ce5.js";const l={},u=a(`<h1 id="grpc教程-3-流式grpc与错误处理" tabindex="-1"><a class="header-anchor" href="#grpc教程-3-流式grpc与错误处理" aria-hidden="true">#</a> GRPC教程 3- 流式GRPC与错误处理</h1><p>::: info</p><ul><li>解决vscode中，一个目录下多个mod文件的问题，怎么解决呢？ 如何 在一个目录下正常的有多个.mod 文件</li></ul><p>使用 go work init 命令 :::</p><p>本篇文章开始，我们将要开始学习流式GRPC与GRPC的错误处理</p><h2 id="流式grpc" tabindex="-1"><a class="header-anchor" href="#流式grpc" aria-hidden="true">#</a> 流式GRPC</h2><p>什么是流式GRPC呢？</p><p>和之前我们写的普通的RPC服务写入直接返回不同，流式GRPC允许我们在一个RPC请求中建立一个Stream(流)，客户端和服务器端都可以向这个流中写入数据，当客户端写入数据时，服务器端只需要不断监听这个流就可以不断获取客户端发送的消息，直到关闭。</p><p>首先我们先说说HTTP/2,GRPC的底层就是HTTP/2协议，HTTP2支持服务器端主动向客户端去发送流数据。</p><p>举例：</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token comment">// proto文件，在Greetering服务中加一行</span>
...
GRPC对比WebSocket
GRPC的错误处理
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,9);function d(v,m){const t=e("ExternalLinkIcon");return o(),c("div",null,[u,s("p",null,[n("GRPC自己定义了一些常见的错误码，和我们可以在"),s("a",r,[n("codes"),i(t)]),n("找到。 需要使用时，需要引入codes包")]),k])}const g=p(l,[["render",d],["__file","3.html.vue"]]);export{g as default};
