import{_ as n,o as s,c as a,e}from"./app-927a6ce5.js";const t={},i=e(`<h1 id="grpc教程-4-grpc-gateway教程与transcoding" tabindex="-1"><a class="header-anchor" href="#grpc教程-4-grpc-gateway教程与transcoding" aria-hidden="true">#</a> GRPC教程 4 - GRPC-Gateway教程与Transcoding</h1><p>:::info GRPC-Gateway是protoc的一个插件，类似protoc-gen-go和protoc-gen-go-grpc插件，前者是生成.pb.go后者是生成.grpc.pb.go文件。 :::</p><p>那么这两个插件是帮助proto文件生成go语言的插件，那么GRPC-Gateway呢，它是一个可以根据proto文件的定义生成一个反向代理器的，服务器可以将 RESTful JSON API 转换为 GRPC。</p><p>这里你会有三个疑惑？什么是反向代理器？为什么要根据proto文件生成反向代理器？为什么要将RESTful JSON API转换成GRPC？</p><p>其实这一个图就可以解释</p><p><img src="https://ts1.cn.mm.bing.net/th/id/R-C.37541e0178c028842f13f773275f5cb3?rik=rZAPx2ssc92Fpg&amp;pid=ImgRaw&amp;r=0" alt=""></p><h2 id="基本示例" tabindex="-1"><a class="header-anchor" href="#基本示例" aria-hidden="true">#</a> 基本示例</h2><p>我们继续使用我们一开始的hello.proto文件来做示例</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>├── client
│   ├── go.mod
│   ├── main.go
│   └── pb
│       ├── hello_grpc.pb.go
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这里 google/api/annotations.proto 却是在go/include/下，也就是我们下载proto时，解压下来的文件夹。和include 同一目录。所以我们需要去跟github上去下载。</p><p>https://github.com/googleapis/googleapis/tree/master/google/api</p><p>随后需要重新加上grpc-gateway插件器创建对应的pb.gw.go文件。</p><p>在这里你需要下载grpc-gateway</p><div class="language-card line-numbers-mode" data-ext="card"><pre class="language-card"><code>go install github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-grpc-gateway@v2
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>总结</p><p>我们本篇文章，主要是介绍了GRPC-Gateway,下载gatwway插件，学习使用gateway,并且学习了传递HTTP的多种方式</p>`,50),l=[i];function o(p,c){return s(),a("div",null,l)}const d=n(t,[["render",o],["__file","4.html.vue"]]);export{d as default};
